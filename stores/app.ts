...
import type {responseRequests, requestParams, appResponses} from '~/types/appTypes'
...
export const useAppStore = defineStore('appStore', () => {
	...
	const getResponse = async({url, data}:requestParams): Promise<appResponses>=>{
		try{
			...
			return {message:'Ошибка получения данных', success:false, responseRow:[]};
		}
		catch(error:any){
	...
	const sendRequest = async(url:string, method:'post' | 'put' | 'delete', data:object): Promise<responseRequests>=>{
		try{
