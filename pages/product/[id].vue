...
const updateQuantity = (change: number) => {
	...
	if (
		newQuantity >= 1 &&
		newQuantity <= (data.value?.productRow[0]?.quantity || 0)
	) {
...
const { data, pending } = useAsyncData(`product-${productId}`, async () => {
	try {
		const data = await $fetch<responseProductPage>(
		...
		<div class="container">
			...
			<div class="breadcrumb">
				<span class="breadcrumb-text"
					>Категория: товары для здоровья</span
				>
			</div>
					...
					<div class="info-section">
						<div class="product-header">
							...
							<div class="rating-container">
								...
								<span class="reviews-count">222 отзыва</span>
							</div>
						</div>
