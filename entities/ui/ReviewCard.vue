...
<div class="reviewCard-container border border-gray-300 rounded-xl max-w-[725px] sm:p-5">
    <div class="review-main-info-container">
        <div class="review-main-info-1c">
            <NuxtImg :src="'https://i.pinimg.com/736x/11/1c/7f/111c7f70a48c84f490574de35e7cb4c5.jpg'" 
            class="reviewAvatar"
            ></NuxtImg>
            <div class="grid">
                <div class="flex items-center gap-[10px]">
                    <h2 class="review-username">{{ reviewCard.reviewUsername }}</h2>
                    <h2 class="reviewCreatedAt text-[13px] sm:text-sm">{{ formatDate(parseDate(reviewCard.createdAt)) }}</h2>
                </div>
                <div class="flex">
            ...
            >
                ...
                >
                    <path
                        ...
                        :fill="(reviewCard.rating >= star) ? '#FFAC33' : '#B0B0B0'"
                    />
                </svg>
                </div>
                </div>
        ...
        <div class="review-main-info-2c">
            <div class="review-reportBtn flex items-center gap-1.25 transition-colors p-2 rounded-xl">
                <svg class="w-[15px] h-[15px] sm:w-[17px] sm:h-[17px]" width="59" height="68" viewBox="0 0 59 68" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <h2 class="sm:block hidden text-[15px]">Пожаловаться</h2>
            </div>
        </div>
    </div>
    <div class="reviewComment-container mt-2.5 text-sm sm:text-base">
        <h2>{{ reviewCard.comment }}</h2>
    </div>
</div>
</template>
...
const props = defineProps<{
    reviewCard:userReviewRows
}>()

const reviewCard = computed(()=> props.reviewCard);
</script>
<style>
