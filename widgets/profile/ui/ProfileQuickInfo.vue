<template>
	<div class="grid grid-cols-2 gap-4">
		<!-- Selling goods quantity -->
		<profile-info-card>
			<template #title>5 товаров </template>
			<template #subTitle>Ждут оценки</template>
			<template #icon
				><svg
		...
		<profile-info-card>
			<template #title>{{
				user?.productsInCart !== undefined
					? pluralize(user?.productsInCart, 'товар')
					: '0 товаров'
			}}</template>
		...
		<profile-info-card>
			<template #title>5 товаров </template>
			<template #subTitle>К выдаче</template>
			<template #icon
				><svg
		...
		<profile-info-card>
			<template #title>5 товаров </template>
			<template #subTitle>Куплено</template>
			<template #icon
				><svg
...
import ProfileInfoCard from '~/entities/ui/profile/ProfileInfoCard.vue'
